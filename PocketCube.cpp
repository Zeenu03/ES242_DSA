...
bool checkvalid(vector<string> input){              //Checks that if the input is solved or not
...
vector<string> solve(vector<string>& input,vector<int>& position,vector<int>& orentationCubes){
    ...
    while(!q.empty()){
        ...
        if(checkvalid(u)){
            vector<string> moves;
            auto o=ord(pos,orent);
            while(visited[o]!="Starting"){
        ...
        inputF=p1.second;
        if(visited[ord(posF,orentF)]==""){
            visited[ord(posF,orentF)]="F";
            parent[ord(posF,orentF)]=ord(pos,orent);
            q.push(inputF);
...
int main(){
    ...
    cout<<"Total number of moves required are "<<move.size()<<endl;
    for(auto i: move){
        cout<<i<<" ";
    }
    return 0;
